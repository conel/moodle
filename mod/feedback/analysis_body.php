...
foreach($_POST as $k => $v) if($v=='') unset($_POST[$k]);
   ...
   print_header_simple(format_string($feedback->name), "", $navigation, "", "", true, $buttontext, navmenu($course, $cm));
	...
	echo "<h1 class=\"center\">".$feedback->name."</h1>";
   ...
   if (isset($_GET['noresponses']) && $_GET['noresponses'] == 1) {
       ...
       echo '<a href="'.$current_page.'?id=' . $id . '&courseid='.$courseid.'&amp;sesskey='.$USER->sesskey.'&amp;reset=1">'.get_string('show_all', 'feedback').'</a>';
       ...
       echo '<a href="'.$current_page.'?id=' . $id . '&amp;courseid='.$courseid.'&amp;reset=1">'.get_string('show_all', 'feedback').' courses</a>';
       ...
       if ($courseitemfilter > 0 && $courseitemfiltertyp != '') {
           ...
           $show_directorates = ($filter_choice == "filter_directorate") ? 'style="display:block;" ' : 'style="display:none;';
           ...
           if ($directorates !== false) {
               ...
               echo html_writer::select($directorates, 'directorates', $search_directorates, 'choose', array('onchange'=>'this.form.submit()'));
               echo '</div>';
           }
			

           if ($search_directorates > 0 && $filter_choice == 'filter_directorate') {
               $directorate_courses = $ff->getCoursesInCategory($search_directorates, $searchcourse);
               if ($directorate_courses !== false) {
                   ...
                   echo html_writer::select($directorate_courses, 'coursefilter', $in_charge, 'choose', array('onchange'=>'this.form.submit()'));
                   echo '</div>';
               } else {
           ...
           $show_school = ($filter_choice == "filter_school") ? 'style="display:block;" ' : 'style="display:none;';
           ...
           if ($schools !== false) {
               ...
               echo html_writer::select($schools, 'schools', $search_schools, 'choose', array('onchange'=>'this.form.submit()'));
               echo '</div>';
           }

           // Need to check if any school filters have been added
           if ($search_schools > 0 && $filter_choice == 'filter_school') {
               $school_courses = $ff->getCoursesInCategory($search_schools, $searchcourse);
               if ($school_courses !== false) {
                   ...
                   echo html_writer::select($school_courses, 'coursefilter', $in_charge, 'choose', array('onchange'=>'this.form.submit()'));
                   echo '</div>';
               } else {
                   echo '<p><b>No courses found</b></p>';
               }
        ...
        $show_level = ($filter_choice == "filter_level") ? 'style="display:block;" ' : 'style="display:none;';
        ...
        echo html_writer::select($levels_arr, 'levels', $search_levels, 'choose', array('onchange'=>'this.form.submit()'));
        echo '</div>';

        if ($search_levels != '0' && $filter_choice == 'filter_level') {

            $level_courses = $ff->getCoursesInLevel($search_levels);
            ...
            if ($level_courses !== false) {
                ...
                echo html_writer::select($level_courses, 'coursefilter', $in_charge, 'choose', array('onchange'=>'this.form.submit()'));
                echo '</div>';
            } else {
                echo '<p><b>No courses found</b></p>';
            }
        ...
        $show_curric = ($filter_choice == "filter_curriculum_area") ? 'style="display:block;" ' : 'style="display:none;';
        ...
        if ($curriculum_areas !== false) {
            ...
            echo html_writer::select($curriculum_areas, 'curriculum_areas', $search_curriculum_areas, 'choose', array('onchange'=>'this.form.submit()'));
            echo '</div>';
        }
        ...
        if ($search_curriculum_areas > 0 && $filter_choice == 'filter_curriculum_area') {
		   ...
		   $subcats = $ff->getCurriculumSubMenu($search_curriculum_areas);
		   ...
		   if ($subcats !== false) {
			   ...
			   echo html_writer::select($subcats, 'subcategory', $search_subcat, 'choose', array('onchange'=>'this.form.submit()'));
			   echo '</div>';
		   }

			$cat_to_search = ($search_subcat > 0) ? $search_subcat : $search_curriculum_areas;
			$curric_courses = $ff->getCoursesInCategory($cat_to_search, $searchcourse);

			if (count($curric_courses) > 0) {

				echo '<div class="filter_curriculum_course">Course: ';
				//choose_from_menu($curric_courses, 'coursefilter', $in_charge, 'choose', 'this.form.submit()');
				echo html_writer::select($curric_courses, 'coursefilter', $in_charge, 'choose', array('onchange'=>'this.form.submit()'));
				echo '</div>';
        ...
        if ($filter_choice == "" || $filter_choice == "filter_course") {
            ...
            $show_course = ($filter_choice == "filter_course") ? 'style="display:block;" ' : 'style="display:none;';
            
            echo '<div id="course_holder"'.$show_course.'>';

            $courses_found = $ff->getCoursesWithAnswers($searchcourse);
            if ($courses_found !== false) {
				$found_courses = $ff->formatCoursesForDropdown($courses_found);
				echo get_string('filter_by_course', 'feedback') . ': ';
				//choose_from_menu($found_courses, 'coursefilter', $in_charge, 'choose', 'this.form.submit()');
				echo html_writer::select($found_courses, 'coursefilter', $in_charge, 'choose', array('onchange'=>'this.form.submit()'));
				//echo'<br /><br />';
            } else {
                echo '<p><b>No courses found</b></p>';
            }
        ...
        echo '<table id="secondary_filter">';
	   ...
	   $mappings = $ff->getFilterIDs();
	   // Create variables from found filter IDs
	   // $site_id, $gender_id, $age_id, $ethnic_id, $attendance_id, $dld_id, $dldb_id
	   extract($mappings, EXTR_OVERWRITE);

        //print the items in an analysed form

		if ($has_site) $ubs = array(); // users by gender site
		...
		if ($has_site) {
			if (($filter_site != '' && $site_id != '')) {
				$completed_ids_to_search = $ubs = $ff->getUsersByQandA($site_id, $filter_site);
			}
		}
        // Gender
        if (($filter_gender != '' && $gender_id != '')) {
            $completed_ids_to_search = $ubg = $ff->getUsersByQandA($gender_id, $filter_gender);
        }
        // Age
        ...
        if (($filter_attendance != '' && $attendance_id != '')) {
            $completed_ids_to_search = $ubat = $ff->getUsersByQandA($attendance_id, $filter_attendance);
        }
        // Declared Learning Difficulty
        if (($filter_learning_difficulty != '' && $dld_id != '')) {
            $completed_ids_to_search = $ubdld = $ff->getUsersByQandA($dld_id, $filter_learning_difficulty);
        }
        // Declared Disability
        if (($filter_learning_disability != '' && $dldb_id != '')) {
            $completed_ids_to_search = $ubdldb = $ff->getUsersByQandA($dldb_id, $filter_learning_disability);
        }
        ...
        $no_courses_error = 'No results match this level of filtering, please redefine your filter options.';
        ...
        if ($filter_site != '' || $filter_gender != '' || $filter_age != '' || $filter_ethnicity != '' || $filter_attendance != '' || $filter_learning_difficulty != '') {
			if ($completed_ids_to_search !== false) {
				$completed_ids = implode(',', $completed_ids_to_search); // convert array to csv format
				$and_clause .= ' AND fbv.completed IN ('.$completed_ids.')';
			} else {
				$no_feedback_error = $no_courses_error;
			}
        }


        if ($search_directorates > 0) {
            if ($to_search = $ff->getCourseIDsAsCSV($directorate_courses)) {
				$and_clause .= ' AND fbv.course_id IN ('.$to_search.')';
			} else {
				$no_feedback_error = $no_courses_error;
				$courses_found = false;
			}
        } else if ($search_schools > 0) {
            if ($to_search = $ff->getCourseIDsAsCSV($school_courses)) {
				$and_clause .= ' AND fbv.course_id IN ('.$to_search.')';
			} else {
				$no_feedback_error = $no_courses_error;
				$courses_found = false;
			}
		} else if ($search_levels != '0') {
            if ($to_search = $ff->getCourseIDsAsCSV($level_courses)) {
				$and_clause .= ' AND fbv.course_id IN ('.$to_search.')';
			} else {
				$no_feedback_error = $no_courses_error;
				$courses_found = false;
			}
        } else if ($search_curriculum_areas > 0) {
            if ($to_search = $ff->getCourseIDsAsCSV($curric_courses)) {
				$and_clause .= ' AND fbv.course_id IN ('.$to_search.')';
			} else {
				$no_feedback_error = $no_courses_error;
				$courses_found = false;
		...
		//fucking bugfix 2
		//if there is nothing inside the paranthesis then we put a 0 inside to prevent db error
		if(($brpos=strpos($and_clause,'()'))!==False) $and_clause=substr($and_clause,0,$brpos+1).'0'.substr($and_clause,$brpos+1);
        ...
        $completed_count = $ff->get_completeds_group_count($feedback, $mygroupid, $coursefilter, $and_clause);
        $completed_count = ($completed_count === false) ? 0 : $completed_count;

		echo '<b>'.get_string('completed_feedbacks', 'feedback').': '.number_format($completed_count). '</b>';
		if ($total_complete != $completed_count) { echo ' of '.number_format($total_complete); }
		echo '<br />';
		...
		if(has_capability('mod/feedback:viewreports', $context)){
            ...
            //print_single_button('analysis_to_excel_filter.php', $export_button_options, $export_button_label, 'post');
			
			print $OUTPUT->single_button(
				new moodle_url('analysis_to_excel_filter.php', $export_button_options), 
				$export_button_label
			);			
        }
        ...
        if ($courses_found === true && $completed_count > 0) {
			...
			function __autoload($class_name) {
				...
				global $CFG;
				include($CFG->dirroot.'/mod/feedback/item/'.$class_path[2].'/lib.php');	
			}
