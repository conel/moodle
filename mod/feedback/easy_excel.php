<?php // $Id: easy_excel.php,v 1.7.2.4 2008/06/27 07:19:15 agrabs Exp $
	...
	*/
	...
	class EasyWorksheet extends Spreadsheet_Excel_Writer_Worksheet {
		 ...
		 *  <f> = Fett == fat (bold?)
		 *  <k> = kursiv == cursive
		 *  <z> = zentriert == centered
		 *  <l> = linksbündig == left
		 *  <r> = rechtsbündig == right
		 *  <vo> = vertikal oben == vertical top aligned
		 *  <vz> = vertikal zentriert == vertical centred
		 *  <vu> = vertikal unten == vertical below
		 *  <uX> = unterstrichen X=1-einfach, X=2-doppelt == underlined X = 1-easy, X = 2-double
		 *  <w> = währungsformat == currency format
		 *  <pr> = prozentformat == percent format
		 *  <ruX> = Rahmen unten X=Stärke == Under the bottom X = strength
		 *  <roX> = rahmen oben X=Stärke == top frame X = strength
		 *  <rrX> = rahmen rechts X=Stärke == right frame X = strength
		 *  <rlX> = rahmen links X=Stärke == left frame X = strength
	...
	function feedback_convert_to_win($text) {
        ...
        //$text = textlib::convert($text, 'utf-8', 'utf-16le');

        //$text = textlib::convert($text, 'utf-8', 'windows-1252');
		...
		*/
		...
		if(!isset($oldcharset)) {
			$oldcharset = 'utf-8'; //get_string('thischarset');
		}
