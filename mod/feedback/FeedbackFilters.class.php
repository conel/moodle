...
class FeedbackFilters {
	...
	public function get_completeds_group_count($feedback, $groupid = false, $courseid = false, $and_clause = false) {
		global $DB;

		$query = 'SELECT DISTINCT fbc.*
				  FROM '.$this->CFG->prefix . 'feedback_completed AS fbc, '.$this->CFG->prefix . 'feedback_value AS fbv
		...
		if ($values = $DB->get_records_sql($query)) {
    ...
    public function getFeedbackItems() {
        ...
        return $DB->get_records_select(
            'feedback_item', 'feedback = '. $this->feedback->id .' AND hasvalue = 1', array(), 'position'
        );
        //return $DB->get_records_select('feedback_item', array('feedback'=>$this->feedback->id, 'hasvalue' => 1), 'position');
    }
    ...
    public function getCourseAvgForQuestion($question_id='', $question_type='') {
        ...
        if ($courses = $DB->get_records_sql($query)) {
            return $courses;
        } else {
    ...
    public function getCurriculumMenu() {
        ...
        if ($curric_cats = $DB->get_records_sql_menu($query)) {
            return $curric_cats;
        } else {
    ...
    public function getCurriculumSubMenu($search_curriculum_areas) {
		
		global $DB;
		
		$query = sprintf(
			"SELECT id, name FROM ".$this->CFG->prefix."course_categories 
		     WHERE parent = (%s) AND visible = 1 
		     ORDER BY name ASC",
		     $search_curriculum_areas
		);
	...
	private function getOldIDNumbers(array $courses) {
		...
		$old_id_numbers = $DB->get_records_sql($query);
    ...
    public function getSubcategories($category_id='') {
        ...
        $paths = $DB->get_records_sql($query, array('?'));
    ...
    private function getQuestions() {
        ...
        if ($questions = $DB->get_records_sql($query)) {
            return $questions;
        } else {
    ...
    public function getUsersByQandA($question_id, $answer_value) {
        ...
        if ($users_by_qa = $DB->get_records_sql($query)) {
