...
class feedback_item_multichoice2 extends feedback_item_multichoice {
	...
	*/
    ...
    //this prevents some output before the excel-header will be sent
    ob_start();
    $fstring = new object();
    ...
    $items = $DB->get_records_select('feedback_item', 'feedback = '. $feedback->id . ' AND hasvalue = 1', null, 'position');
    if(is_array($items)){
        $rowOffset1++;
    ...
    foreach($items as $item) {
        ...
        $itemclass = 'feedback_item_'.$item->typ.'2';
        //get the instance of the item-class
        $itemobj = new $itemclass();
		if ($and_clause != '') {
			$and_clause_where = "1=1" . $and_clause . "AND fbv.item = ".$item->id;
        } else {
			$and_clause_where = "1=1 AND fbv.item = ".$item->id;
        }
		$rowOffset1 = $itemobj->excelprint_item($worksheet1, $rowOffset1, $item, $mygroupid, $coursefilter, $and_clause_where);
    }
