...
function stats_cron_daily($maxdays=1) {
	...
	echo "last time stats daily ran = ".date("F j, Y, g:i a",$timestart)."\n";

    // calculate scheduled time
    $scheduledtime = stats_get_base_daily() + $CFG->statsruntimestarthour*60*60 + $CFG->statsruntimestartminute*60;
	
	echo "Scheduled time = ".date("F j, Y, g:i a",$scheduledtime)." \n";
	...
	*/
	...
	echo "Next midnight = ".date("F j, Y, g:i a",$nextmidnight)." \n";
	...
	*/
	...
	echo "Time start = ".date("F j, Y, g:i a",$timestart)."($timestart) \n";
	echo "SQL: SELECT * FROM mdl_stats_daily where timeend > $timestart";
...
function stats_get_start_from($str) {
    ...
    if ($timeend = $DB->get_field_sql('SELECT timeend FROM '.$CFG->prefix.'stats_'.$str.' ORDER BY timeend DESC')) {
        return $timeend;
    }
    // decide what to do based on our config setting (either all or none or a timestamp)
    switch ($CFG->statsfirstrun) {
        case 'all':
            if ($firstlog = $DB->get_field_sql('SELECT time FROM '.$CFG->prefix.'log ORDER BY time ASC')) {
                return $firstlog;
            }
        default:
...
function stats_get_parameters($time,$report,$courseid,$mode,$roleid=0) {
    ...
    if ($time == 0) {
        ...
        $param->timeafter = mktime (0,0,0,date("m"),date("d")-$time,  date("Y"));
    }
    $param->extras = '';
    ...
    // nkowald - 2010-06-27 - Need to create my own to handle new named reports (stat_types)
    case 'all':
    case 'views':
    case 'adds':
    case 'updates':
    case 'uploads':
    case 'deletes':
        $param->fields = $DB->sql_concat('time_end','role_id').' AS uniqueid, time_end, role_id, stat1 as line1';
        $param->fieldscomplete = true; // set this to true to avoid anything adding stuff to the list and breaking complex queries.
        $param->aggregategroupby = 'role_id';
        $param->stattype = $report;
        $param->crosstab = true;
        $param->extras = 'GROUP BY time_end,role_id,stat1';
        if ($courseid == SITEID) {
            $param->fields = $DB->sql_concat('time_end','role_id').' AS uniqueid, time_end, role_id, sum(stat1) as line1';
            $param->extras = 'GROUP BY time_end,role_id';
        }
        break;
    ...
    case STATS_REPORT_ACTIVITY:
        $param->fields = $DB->sql_concat('time_end','role_id').' AS uniqueid, time_end, role_id, activity as line1';
        $param->fieldscomplete = true; // set this to true to avoid anything adding stuff to the list and breaking complex queries.
        $param->aggregategroupby = 'role_id';
        $param->stattype = 'activity';
        $param->crosstab = true;
