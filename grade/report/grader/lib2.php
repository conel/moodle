...
class grade_report_grader2 extends grade_report_grader {
    
    function grade_report_grader2($courseid, $module, $gpr, $context, $page=null, $sortitemid=null) {
		...
		//print "canviewhidden: " . $this->canviewhidden . "<br>";
		//exit;
		...
		//print "nooutcomes: " . $nooutcomes ? "true" : "false" . "<br>";
		//exit;
    ...
    function load_users() {
        ...
        //$this->users = get_records_sql($sql, $this->get_pref('studentsperpage') * $this->page, $this->get_pref('studentsperpage'));
        $this->users = $DB->get_records_sql($sql, null, $this->get_pref('studentsperpage') * $this->page, $this->get_pref('studentsperpage'));
		...
		// get e-zone admins ids
		$query = "SELECT userid FROM mdl_role_assignments WHERE roleid = 1";
		$ezone_admin_ids = $DB->get_records_sql($query);
		foreach($ezone_admin_ids as $admin) {
			$ezone_admins[] = $admin->userid;
		}
    ...
    function get_headerhtml() {
		...
		//print "showuseridnumber: $showuseridnumber<br>";
        ...
        foreach ($this->gtree->levels as $key => $row) {
			...
			//print_object($headerhtml);
			
			if ($key == $numrows - 1) {
				$headerhtml .='<th class=" c0" scope="col">First name / Surname</th>';
            ...
            foreach ($row as $columnkey => $element) {
				...
				if (!empty($element['object']->scaleid)) {
					$scale = $DB->get_record('scale', array('id' => $element['object']->scaleid));
					//print_object($scale);
					$scales = explode(',', $scale->scale);							
				}
    ...
    function get_studentshtml() {
        ...
        if (!empty($scales_list)) {
            $scales_list = explode(',', substr($scales_list, 0, -1));
        ...
        foreach ($this->users as $userid => $user) {
            ...
            if ($fixedstudents) {
                ...
                $studentshtml .= '<tr class="r'.$this->rowcount++ . $row_classes[$this->rowcount % 2] . '">'
                              .'<th style="white-space:normal !important;" class="c'.$columncount++.' user" scope="row" onclick="set_row(this.parentNode.rowIndex);">'.$user_pic
                              .'<a href="'.$CFG->wwwroot.'/user/view.php?id='.$user->id.'&amp;course='.$this->course->id.'">'
                              .fullname($user).'</a></th>';
			...
			//print "studentshtml:";
			//print_object($studentshtml);
			
			//insert progress bar	
			
			//print "TABLE_VAR_SORT: ".TABLE_VAR_SORT.'<br>';
			...
			$courseobject->candidates = $candidatelist;
			$courselist = array($this->courseid => $courseobject);

			$flextable = new assmgr_ajax_table('f');
			
			$flextable->setup=1;
			
			$dbc = new assmgr_db();
			
			$matrix = $dbc->get_portfolio_matrix($candidatelist, $courselist, $flextable, 0);
			...
			$progress_bar_cell = "<div class='progress_bar_cell {$highlight}'>{$cell}</div>";
			
			$studentshtml .= '<td class="cell c'.$columncount++.'">';
			
			$studentshtml .= '<div class="assmgr yui-skin-sam">';
			
			$studentshtml .= $progress_bar_cell;
    ...
    function get_studentnameshtml() {
        ...
        $studentshtml .= '<table id="user-grades" class="gradestable flexible boxaligncenter generaltable"><tbody>';
...
class grade_category2 extends grade_category {
    ...
    private static function _get_children_recursion($category) {
        ...
        foreach($category->children as $sortorder => $child) {
            ...
            if (array_key_exists('itemtype', $child)) {
                ...
                $children = grade_category2::_get_children_recursion($child);
                
                $grade_category = new grade_category2($child, false);
    ...
    public function get_children($module, $include_category_items=false) {
        ...
        ////$items = recordset_to_array($DB->get_recordset_select('grade_items', "courseid='".$this->courseid."' AND (itemmodule='$module' OR ISNULL(itemmodule))"));
        ...
        //$items = recordset_to_array($DB->get_recordset_select('grade_items', "courseid='".$this->courseid."' AND (itemmodule='$module' OR ISNULL(itemmodule))"));
        //$items = iterator_to_array($DB->get_recordset_select('grade_items', "courseid='".$this->courseid."' AND (itemmodule='$module' OR ISNULL(itemmodule))"));
        $items = $DB->get_recordset_select('grade_items', "courseid='".$this->courseid."' AND (itemmodule='$module' OR ISNULL(itemmodule))");
        
        //print_object($items);
        //print_object(iterator_to_array($items));
    ...
    public static function fetch_course_tree($courseid, $module, $include_category_items=false) {
...
class grade_tree2 extends grade_tree {
    ...
    function grade_tree2($courseid, $module, $fillers=true, $category_grade_last=false, $collapsed=null, $nooutcomes=false) {
		...
		//print "grade tree top element<br>";		
		//print_object($this->top_element);
		//exit;
