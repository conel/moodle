            ...
            <tr>
                ...
                <?php  
                    ...
                    if ($view != 'course') {
                        ...
                        if ($directorates = $DB->get_records_sql_menu($query)) {
														
                            $show_style = ($current_filter == 'directorate') ? ' style="display:block;" ' : ' style="display:none;" ';
							...
							echo html_writer::select($directorates, 'did', $filter_category); //, 'choose', array('onchange'=>'this.form.submit()'));
                            echo '</p>';
                        }
                        ...
                        if ($schools = $DB->get_records_sql_menu($query)) {
                            $show_style = ($current_filter == 'school') ? ' style="display:block;" ' : ' style="display:none;" ';
                            echo'<p id="select_schools"'.$show_style.'><label>School:</label> ';
                            //choose_from_menu($schools, 'sid', $filter_category, 'choose', '');
                            echo html_writer::select($schools, 'sid', $filter_category, 'choose'); //, array('onchange'=>'this.form.submit()'));
                            echo '</p>';
                        }
                        ...
                        if ($curriculum_areas = $DB->get_records_sql_menu($query)) {
                            $show_style = ($current_filter == 'curriculum_area') ? ' style="display:block;" ' : ' style="display:none;" ';
                            ...
                            echo html_writer::select($curriculum_areas, 'cid', $filter_category, 'choose'); //, array('onchange'=>'this.form.submit()'));
                            echo '</p>';
                        }
                        ...
                        // If filter_category chosen, show drop down of its subcategories
                         if ($filter_category != 0) {
                            ...
                            if ($subcats) {
                                ...
                                echo html_writer::select($subcats, 'subcat', $subcat, 'choose');
                                echo '</p>';
                            }
                         }
                         ...
                         if ($subcat != 0) {
                            ...
                            if ($subsubcats) {
                                ...
                                //choose_from_menu($subsubcats, 'subsubcat', $subsubcat, 'choose', 'this.form.form.submit()'));
                                echo html_writer::select($subsubcats, 'subsubcat', $subsubcat, 'choose', array('onchange'=>'this.form.submit()'));
                                echo '</p>';
                            }
                         }
                        ...
                        if (in_array($filter, $valid_filters) && $subcat != 0) {
                            ...
                            if ($courses != '') {
                                ...
                                if ($filtered_courses = get_records_sql_menu($query)) {
                                    ...
                                    echo html_writer::select($courses_array, 'course_id', $filter_course, 'choose', array('onchange'=>'this.form.submit()'));
                                    echo '</p>';
                                }
                            }
                        }
                    ...
                    if ($filter_course != 0) {
                    ...
                    } else if (isset($selected_cat) && $selected_cat != 0) {
						...
						//if ($courses = $DB->get_records('course', 'visible', 1)) {
						if ($courses = $DB->get_records('course', array('visible' => 1))) {
                    ...
                    echo '<div id="type_key" style="display:none">';
                        echo '<h3>Type Key</h3>';
                    ...
                    echo '<div class="graph"><img src="'.$CFG->wwwroot.'/report/stats/graph.php?mode=1&amp;course=1&amp;time='.$days.'&amp;report=4&amp;roleid=0&amp;stat_type='.$stat_type.'&amp;course_id='.$filter_course.'&amp;category_id='.$filter_category.'" alt="Statistics graph" id="stat_graph" width="750" height="400" /></div>';
                    echo '</center>';
                    echo '<br />';
