    ...
    class StatsEnhanced 
    {
		...
		************************************/
        public function checkStatsTableExists() {
            ...
            $query = "SELECT * FROM mdl_stats_new LIMIT 1";
		
            //if (!$table_exists = $DB->execute($query, false)) {
                    ...
                    foreach($count_ac as $key => $value) {
                        ...
                        if (($key == 1 || $key == 2 || $key == 3 || $key == 5)) {
                            ...
                            if ($results = $DB->get_records_sql($query)) {
                                ...
                                $stat_data->id = $row_id;
                                if (!$DB->update_record('stats_new', $stat_data)) {
                                    $this->errors[] = "Stat could not be updated (".date('F Y', $start_date)."), Role ID = $key";
                                }
                            // Insert new stat
                            } else {
                                if (!$DB->insert_record('stats_new', $stat_data)) {
                ...
                if ($i == 0) {
                    echo "<table cellpadding=\"7\" cellspacing=\"2\" border=\"1\" class=\"generaltable boxaligncenter\">
						...
						foreach ($role_ids as $role) {
							$role_name = $DB->get_record('role',array('id'=>$role));
		...
		************************************/
        public function buildLogQuery($start_date=0, $end_date=0, $stat_type='all', $filter_course=0, $filter_category=0) {
			...
			$type_all = '';  //what the fuck is this for????
            ...
            if (in_array($stat_type, $this->valid_stat_types)) {
                switch ($stat_type) {
                    ...
                    case 'deletes':
                        ...
                        break;
                    default:
                        $in_sql = $type_all; //what the fuck is this for????
                }
                if ($stat_type != 'all') {
		...
		************************************/
        public function getName($category_id=0, $course_id=0) {
            ...
            if (is_numeric($category_id) && $category_id != 0) {
                $name = $DB->get_record('course_categories', array('id' => $category_id));
                if ($name->name != '') {
                    return $name->name;
                }
            } else if (is_numeric($course_id) && $course_id != 0) {
                $name = $DB->get_record('course', array('id' => $course_id));
                if ($name->fullname != '') {
